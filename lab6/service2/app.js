...
app.get('/greet/:name?', (req, res) =>
  res.send(`Hi ${req.params.name || 'Guest'}! This is ${serviceName}.`)
);
...
app.listen(port, () => {
  ...
  amqp.connect(rabbitUrl, function (error0, connection) {
    if (error0) {
      console.log('Rabbit connect error (service2):', error0.message || error0);
      return;
    }
    connection.createChannel(function (error1, channel) {
      if (error1) {
        console.log('Channel error (service2):', error1);
        return;
      }
      channel.assertQueue(queue, {durable: false});
      console.log(`${serviceName} waiting for messages in "${queue}"`);
      channel.consume(
        queue,
        function (msg) {
          if (msg) {
